...
const SetDetails = () => {
  ...
  useEffect(() => {
    const fetchBatches = async () => {
      try {
        ...
        setBatches(res.data);
        if (res.data.length > 0 && !formData.batch) {
          setFormData((prev) => ({ ...prev, batch: res.data[0].name }));
        }
      } catch (error) {
  ...
  return (
    <div className="container">
         <button className="button-54" role="button" onClick={() => navigate('/StudentCodesByDate')}>
...
{fatherOptions.length > 1 ? (
  ...
  />
        ...
        <select className="selectcl" name="batch" onChange={handleChange} value={formData.batch} required>
          {batches.map((batch) => (
